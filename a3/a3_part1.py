...
def load_hypertension_data(filename: str) -> list[HypertensionData]:
    ...
    with open(filename) as f:
        ...
        for row in reader:
            ...
            list.append(data_so_far, HypertensionData(row[0], int(row[1]), int(row[2]), int(row[3]), int(row[4]),
...
def load_low_income_data(filename: str) -> list[LowIncomeData]:
    ...
    with open(filename) as f:
        ...
        for row in reader:
            ...
            # Your task is to extract the relevant data from row and add it to the accumulator.
            # Make sure to use type conversion to ensure instance attributes have the correct type.
            list.append(data_so_far, LowIncomeData(row[0], int(row[2]), int(row[1])))
...
def total_num_hypertension(data: list[HypertensionData]) -> int:
    ...
    for row in data:
        total_num_people += row.num_hypertension_all
...
def high_hypertension_rate(data: list[HypertensionData], threshold: float) -> set[str]:
    ...
    for row in data:
        if (row.num_hypertension_all / row.num_all) >= threshold:
            set.add(neighbourhoods, row.name)
...
def get_hypertension_rates(data: list[HypertensionData], age_group: str) -> dict[str, float]:
    ...
    """
    neighbourhood_to_rate = {}

    for row in data:
        if age_group == '20+':
            neighbourhood_to_rate[row.name] = row.num_hypertension_all / row.num_all
        elif age_group == '20-44':
            neighbourhood_to_rate[row.name] = row.num_hypertension_20_44 / row.num_20_44
        elif age_group == '45-64':
            neighbourhood_to_rate[row.name] = row.num_hypertension_45_64 / row.num_45_64
        else:
            neighbourhood_to_rate[row.name] = row.num_hypertension_65_plus / row.num_65_plus

    return neighbourhood_to_rate
...
def combine_rates(hypertension_data: list[HypertensionData],
    ...
    for neighbourhood in hypertension_rates:
        if neighbourhood in low_income_rates:
            list.append(combined_rates, CombinedRateData(neighbourhood, hypertension_rates[neighbourhood],
                                                         low_income_rates[neighbourhood]))

    return combined_rates


def get_low_income_rates(low_income_data: list[LowIncomeData]) -> dict[str, float]:
    """Return a dictionary mapping each given neighbourhood's name to the neighbourhood's low income rate.

    Preconditions:
    - data does not contain any duplicated neighbourhood names

    >>> data = load_low_income_data('datasets/part1/low_income_data_small.csv')
    >>> result = get_low_income_rates(data)
    >>> len(result)
    5
    ...
    for row in low_income_data:
        neighbourhood_to_rate[row.name] = row.num_low_income / row.population_total
...
def part1_example(hypertension_file: str, low_income_file: str, age_group: str) -> None:
    ...
    combined_neighbourhood_data = combine_rates(hypertension_data, low_income_data, age_group)
    plot_combined_rates(combined_neighbourhood_data, age_group)
