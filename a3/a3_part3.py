...
def part3_warmup() -> None:
    ...
    for i in range(0, 100):
        a3_helpers.draw_pixel(screen, (400, 200 + i), (0, 0, 0))
...
def generate_point_sequence1(vertex_points: list[tuple[int, int]],
    ...
    True
    """
    point_sequence = [initial_point]

    for i in range(1, num_points):
        random_vertex_index = random.randint(0, len(vertex_points) - 1)
        random_vertex = vertex_points[random_vertex_index]

        # Cacluate the next point and add it to the accumulator
        midpoint_x = (point_sequence[i - 1][0] + random_vertex[0]) // 2
        midpoint_y = (point_sequence[i - 1][1] + random_vertex[1]) // 2
        next_point = (midpoint_x, midpoint_y)
        list.append(point_sequence, next_point)
...
def draw_point_sequence1(
    ...
    # Keep track of the last point drawn
    last_point_drawn = initial_point
    ...
    for _ in range(1, num_points):
        ...
        # Cacluate the next point and add it to the accumulator
        midpoint_x = (last_point_drawn[0] + random_vertex[0]) // 2
        midpoint_y = (last_point_drawn[1] + random_vertex[1]) // 2
        new_point = (midpoint_x, midpoint_y)

        # Draw the pixel
        a3_helpers.draw_pixel(screen, new_point, (0, 0, 0))
        last_point_drawn = new_point
...
def generate_point_sequence2(vertex_points: list[tuple[int, int]],
    ...
    """
    # Accumulator for the point sequence generated, initialized with the initial point already at index zero
    point_sequence = [initial_point]

    # Since point_sequence starts with initial_point as an element, the list of indexes of the vertexes will always be
    # an extra index behind the points, i.e. the point in point_sequence[1] would have been chosen using the vertex
    # index at indexes_of_vertex_points[0]
    indexes_of_vertex_points = []

    for i in range(1, num_points):
        if i >= 3 and indexes_of_vertex_points[i - 2] == indexes_of_vertex_points[i - 3]:
            # Offset by at least 2 and at most up to the length of the vertex points minus 2
            random_offset = random.randint(2, len(vertex_points) - 2)
            random_vertex_index = (indexes_of_vertex_points[i - 2] + random_offset) % len(vertex_points)
        else:
            random_vertex_index = random.randint(0, len(vertex_points) - 1)

        list.append(indexes_of_vertex_points, random_vertex_index)
        random_vertex = vertex_points[random_vertex_index]
...
def draw_point_sequence2(
    ...
    # Keep track of the last point drawn and indexes of vertex points
    last_point_drawn = initial_point
    indexes_of_vertex_points = []

    for i in range(1, num_points):
        if i >= 3 and indexes_of_vertex_points[i - 2] == indexes_of_vertex_points[i - 3]:
            # Offset by at least 2, up to the length of the vertex points minus 1
            random_offset = random.randint(2, len(vertex_points) - 2)
            random_vertex_index = (indexes_of_vertex_points[i - 2] + random_offset) % len(vertex_points)
        else:
            random_vertex_index = random.randint(0, len(vertex_points) - 1)
...
def verify_point_sequence1(vertex_points: list[tuple[int, int]],
    ...
    for i in range(0, len(points) - 1):
        if points[i + 1] not in generate_possible_midpoints(points[i], vertex_points):
            return False

    return True


def generate_possible_midpoints(current_point: tuple[int, int],
                                vertex_points: list[tuple[int, int]]) -> list[tuple[int, int]]:
    """Return the possible midpoints between the given current_point and the given vertex_points. This is a helper
    function for the function verify_point_sequence1

    Preconditions:
    - len(vertex_points) >= 3
    - vertex_points does not contain duplicates
    """
    possible_midpoints_so_far = []

    for point in vertex_points:
        midpoint_x = (current_point[0] + point[0]) // 2
        midpoint_y = (current_point[1] + point[1]) // 2
        midpoint = (midpoint_x, midpoint_y)
        list.append(possible_midpoints_so_far, midpoint)

    return possible_midpoints_so_far
...
def user_pattern(screen_width: int, screen_height: int, num_vertices: int, sequence_type: int, num_points: int) -> None:
    ...
    for _ in range(0, num_vertices):
        list.append(vertex_points, a3_helpers.input_mouse_pygame())
    ...
    if sequence_type == 1:
        ...
        draw_point_sequence2(screen_width, screen_height, vertex_points, initial_point, num_points)

    # The functions draw_point_sequence1 and draw_point_sequence2 wait for the user to close the window
