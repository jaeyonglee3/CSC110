...
def crop_row_border_single(colour_row: list) -> list:
    ...
    """
    num_colours = len(colour_row) - 2
    return crop_row(colour_row, 1, num_colours)
...
def crop_row_border_multiple(colour_row: list, border_size: int) -> list:
    ...
    with the appropriate arguments. (For extra practice, try both ways!)
    """
    num_colours = len(colour_row) - 2 * border_size
    return crop_row(colour_row, border_size, num_colours)
...
def fade_red_in_row(colour_row: list) -> list:
    ...
    """
    return [[round(colour[0] * 0.25), colour[1], colour[2]] for colour in colour_row]
...
def fade_row(colour_row: list) -> list:
    ...
    """
    row_length = len(colour_row)
    return [[round(colour_row[j][i] * j / row_length) for i in range(0, 3)] for j in range(0, row_length)]
...
def blur_row(colour_row: list) -> list:
    ...
    """
    row_length = len(colour_row)
    return [average_colour(colour_row[j - 1], colour_row[j], colour_row[j + 1]) for j in range(1, row_length - 1)]


def average_colour(colour1: list, colour2: list, colour3: list) -> list:
    """Return a new colour based on the average of the colour values of the three colours.

    >>> average_colour([0, 255, 200], [1, 2, 3], [100, 100, 100])
    [34, 119, 101]
    """
    average_red = round((colour1[0] + colour2[0] + colour3[0]) / 3)
    average_green = round((colour1[1] + colour2[1] + colour3[1]) / 3)
    average_blue = round((colour1[2] + colour2[2] + colour3[2]) / 3)
